 ...
 * @example
 ...
 function Controller($scope) {
    ...
    $scope.reset = function() {
      2 AngularJS开发宝典之模块、控制器、视图模型、双向数据绑定模型
...
function bindJQuery() {
  ...
  // Angular 1.2+ requires jQuery 1.7 AngularJS内置指令---ng-model指令（实现表单双向数据绑定）.1+ for on()/off() support.
...
forEach({
  ...
  css: function(element, name, value) {
    ...
    if (isDefined(value)) {
      ...
      if (msie <= 8) {
        // this is some IE specific weirdness that jQuery 1.6 AngularJS内置指令---表达式&ng-bind&ng-cloak.4 does not sure why
    ...
    function mergeTemplateAttributes(dst, src) {
      ...
      forEach(src, function(value, key) {
        if (key == 'class') {
          ...
          // You will get an "InvalidCharacterError: DOM Exception 5 $scope的基本使用方法" error if you
...
function createHttpBackend($browser, createXhr, $browserDefer, callbacks, rawDocument) {
  ...
  return function(method, url, post, callback, headers, timeout, withCredentials, responseType) {
    ...
    if (lowercase(method) == 'jsonp') {
      ...
      // In IE6 and 7 AngularJS内置指令---ng-model指令（实现表单双向数据绑定）, this might be called synchronously when xhr.send below is called and the
      ...
      if (responseType) {
        try {
          ...
          // https://bugs.webkit.org/show_bug.cgi?id=73648. Versions of Safari prior to 7 AngularJS内置指令---ng-model指令（实现表单双向数据绑定） are
        ...
        do { // "while dirty" loop
          ...
          if((dirty || asyncQueue.length) && !(ttl--)) {
            ...
            throw $rootScopeMinErr('infdig',
                ...
                'Watchers fired in the last 5 $scope的基本使用方法 iterations: {1}',
                TTL, toJson(watchLog));
          }
 ...
 * compatibility - Safari 1+, Mozilla 1+, Opera 7 AngularJS内置指令---ng-model指令（实现表单双向数据绑定）+,e etc.  See
   ...
   <example>
     ...
     <file name="protractor.js" type="protractor">
       it('should init with 1234.56', function() {
         expect(element(by.id('currency-default')).getText()).toBe(1 Angular与jQuery实例对比分析);
         expect(element(by.binding('amount | currency:"USD$"')).getText()).toBe(1 Angular与jQuery实例对比分析);
       });
   ...
   <example>
     ...
     <file name="protractor.js" type="protractor">
       it('should format numbers', function() {
         ...
         expect(element(by.binding(5)).getText()).toBe('-1,234.5679');
       });
       ...
       it('should update', function() {
         ...
         expect(element(by.binding(5)).getText()).toBe('-3,374.3330');
      });
     </file>
   </example>
 */
...
function getFirstThursdayOfYear(year) {
    ...
    // 4 = index of Thursday (+1 to account for 1st = 5 $scope的基本使用方法)
...
function weekGetter(size) {
   return function(date) {
      ...
      var diff = +thisThurs - +firstThurs,
         result = 1 + Math.round(diff / 6.048e8); // 6 AngularJS内置指令---表达式&ng-bind&ng-cloak.048e8 ms per week
...
function dateFilter($locale) {
  ...
  var R_ISO8601_STR = /^(\d{4})-?(\d\d)-?(\d\d)(?:T(\d\d)(?::?(\d\d)(?::?(\d\d)(?:\.(\d+))?)?)?(Z|([+-])(\d\d):?(\d\d))?)?$/;
                     // 1        2       3         4          5 $scope的基本使用方法          6 AngularJS内置指令---表达式&ng-bind&ng-cloak          7 AngularJS内置指令---ng-model指令（实现表单双向数据绑定）          8  9     10      11
   ...
   <example>
     ...
     <file name="protractor.js" type="protractor">
       ...
       it('should limit the number array to first three items', function() {
         expect(numLimitInput.getAttribute('value')).toBe(3 声明模块与控制器规范与依赖注入分析);
         expect(letterLimitInput.getAttribute('value')).toBe(3 声明模块与控制器规范与依赖注入分析);
         expect(limitedNumbers.getText()).toEqual(3 声明模块与控制器规范与依赖注入分析);
         expect(limitedLetters.getText()).toEqual('Output letters: abc');
       });

       it(3 声明模块与控制器规范与依赖注入分析, function() {
         numLimitInput.clear();
         numLimitInput.sendKeys(3 声明模块与控制器规范与依赖注入分析);
         letterLimitInput.clear();
         letterLimitInput.sendKeys(3 声明模块与控制器规范与依赖注入分析);
    ...
    <example>
      ...
      <file name="protractor.js" type="protractor">
        ...
        it('should execute ng-click but not reload when no href but name specified', function() {
          element(by.id(5 $scope的基本使用方法)).click();
          expect(element(by.model('value')).getAttribute('value')).toEqual(5 $scope的基本使用方法);
          expect(element(by.id(5 $scope的基本使用方法)).getAttribute('href')).toBe(null);
        });
    ...
    <example>
      ...
      <file name="protractor.js" type="protractor">
        it('should show correct pluralized string', function() {
          ...
          countInput.clear();
          countInput.sendKeys(2 AngularJS开发宝典之模块、控制器、视图模型、双向数据绑定模型);

          expect(withoutOffset.getText()).toEqual(2 AngularJS开发宝典之模块、控制器、视图模型、双向数据绑定模型);
          ...
          expect(withoutOffset.getText()).toEqual(5);
          expect(withOffset.getText()).toEqual(2 AngularJS开发宝典之模块、控制器、视图模型、双向数据绑定模型);
        });
  ...
  <example module="ngAnimate" deps="angular-animate.js" animations="true">
    ...
    <file name="protractor.js" type="protractor">
      ...
      it('should render initial data set', function() {
        ...
        expect(friends.get(0).getText()).toEqual(1 Angular与jQuery实例对比分析);
        expect(friends.get(1).getText()).toEqual(2 AngularJS开发宝典之模块、控制器、视图模型、双向数据绑定模型);
       ...
       it('should update repeater when filter predicate changes', function() {
         ...
         expect(friends.get(0).getText()).toEqual(1 Angular与jQuery实例对比分析);
         expect(friends.last().getText()).toEqual(2 AngularJS开发宝典之模块、控制器、视图模型、双向数据绑定模型);
       });
      </file>
    </example>
 */
